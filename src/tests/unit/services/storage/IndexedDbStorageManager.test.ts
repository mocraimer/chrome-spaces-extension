...
describe('IndexedDbStorageManager', () => {
  ...
  describe('createSpace', () => {
    it('should create a space with tabs', async () => {
      ...
      const space = await storageManager.createSpace(windowId, name, urls, customName);

      expect(space.id).toBe('123');
      expect(space.name).toBe(customName);
    ...
    it('should create space without custom name', async () => {
      const space = await storageManager.createSpace(456, 'Default Space', ['https://example.com']);

      expect(space.named).toBe(false);
      expect(space.customName).toBeUndefined();
    });
  });

  describe('updateSpaceCustomName', () => {
    it('should update custom name for active space', async () => {
      const space = createMockSpace('1');
      await storageManager.saveSpaces({ '1': space });

      await storageManager.updateSpaceCustomName('1', 'New Custom Name');

      const loaded = await storageManager.loadSpaces();
      expect(loaded['1']?.customName).toBe('New Custom Name');
      expect(loaded['1']?.name).toBe('New Custom Name');
      expect(loaded['1']?.version).toBe(2);
    });

    it('should update custom name for closed space', async () => {
      const space = createMockSpace('uuid-1', { isActive: false });
      await storageManager.saveClosedSpaces({ 'uuid-1': space });

      await storageManager.updateSpaceCustomName('uuid-1', 'Closed Custom Name');

      const loaded = await storageManager.loadClosedSpaces();
      expect(loaded['uuid-1']?.customName).toBe('Closed Custom Name');
      expect(loaded['uuid-1']?.name).toBe('Closed Custom Name');
    });
  });
