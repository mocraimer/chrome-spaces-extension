        ...
        for (const window of windows) {
          if (window.tabs && window.tabs.length > 0) {
            ...
            if (urls.length > 0) {
              ...
              const defaultName = `${DEFAULT_SPACE_NAME} ${windowIdStr}`;
              const customNameEntry = storedCustomNames[permanentId];
              
              const space: Space = {
                id: windowIdStr,
                name: customNameEntry?.customName || defaultName,
    ...
    if (success) {
      ...
      if ('isActive' in space && space.isActive) {
        setSpaces(prev => prev.map(s => 
          s.id === editingSpaceId 
            ? { ...s, name: trimmedName, customName: trimmedName }
            : s
        ));
      } else {
    ...
    switch (e.key) {
      ...
      case 'Escape':
        ...
        if (e.ctrlKey || e.metaKey) {
          e.preventDefault();
          // Reset to the current saved/default name (Ctrl+Z)
          const space = [...spaces, ...closedSpaces].find(s => s.id === editingSpaceId);
          if (space) {
            setEditingName(space.name);
          }
              ...
              >
                <div className="space-info">
                  {editingSpaceId === space.id ? (
                    ...
                    <h3 
                      className={space.customName ? 'custom-name' : 'default-name'}
                      title={space.customName ? 'Custom name - press F2 to edit' : 'Default name - press F2 to edit'}
                    >
